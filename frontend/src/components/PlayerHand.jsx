...
function PlayerHand({ seat, selectedCardIds, onToggleCard }) {
  ...
  const spread = Math.min(40, 320 / count)

  return (
    <div className="relative h-32 w-full max-w-3xl flex items-end justify-center">
        ...
        {sorted.map((card, index) => {
          ...
          return (
            ...
            >
              <div
                className={
                  "w-14 h-20 rounded-2xl border flex flex-col justify-between px-2 py-1.5 bg-slate-50 " +
                  (card.isRedTen
